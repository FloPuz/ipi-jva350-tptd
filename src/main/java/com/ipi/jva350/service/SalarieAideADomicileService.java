...
public class SalarieAideADomicileService {
	...
	/**
	 * Calcule la limite maximale de congés prenable autorisée selon les règles
	 * de l'entreprise, à savoir : - de base, les congés acquis en année N-1 dans
	 * la proportion selon l'avancement dans l'année, pondéré avec poids plus
	 * gros sur juillet et août (20 vs 8), - si la moyenne actuelle des congés
	 * pris diffère de 20% de la précédente limite, bonus ou malus de 20% de la
	 * différence pour aider à équilibrer la moyenne actuelle des congés pris -
	 * marge supplémentaire de 10% du nombre de mois jusqu'à celui du dernier jour
	 * de congé - bonus de 1 par année d'ancienneté jusqu'à 10 Utilisé par
	 * ajouteMois(). NB. ajouteMois() a déjà vérifié que le congé est dans
	 * l'année en cours.
	 * 
	...
	public long calculeLimiteEntrepriseCongesPermis(LocalDate moisEnCours, double congesPayesAcquisAnneeNMoins1,
		...
		// proportion selon l'avancement dans l'année, pondérée avec poids plus gros
		// sur juillet et août (20 vs 8) :
	...
	public void ajouteConge(SalarieAideADomicile salarieAideADomicile, LocalDate jourDebut, LocalDate jourFin)
		...
		LinkedHashSet<LocalDate> joursDecomptes = salarieAideADomicile.calculeJoursDeCongeDecomptesPourPlage(jourDebut,
				jourFin);
		...
		if (joursDecomptes.stream().findFirst().orElse(LocalDate.now().minusMonths(1)).isBefore(salarieAideADomicile.getMoisEnCours())) {
		...
		LinkedHashSet<LocalDate> congesPayesPrisDecomptesAnneeN = new LinkedHashSet<>(joursDecomptes.stream()
				.filter(d -> !d.isAfter(LocalDate.of(
						Entreprise.getPremierJourAnneeDeConges(salarieAideADomicile.getMoisEnCours()).getYear() + 1, 5,
						31)))
		...
		if (joursDecomptes.size() > nbCongesPayesPrisDecomptesAnneeN + 1) {
			// NB. 1 jour dans la nouvelle année est toujours toléré, pour résoudre le
			// cas d'un congé devant se finir un
		...
		double limiteEntreprise = this.calculeLimiteEntrepriseCongesPermis(salarieAideADomicile.getMoisEnCours(),
				salarieAideADomicile.getCongesPayesAcquisAnneeNMoins1(), salarieAideADomicile.getMoisDebutContrat(),
				jourDebut, jourFin);
	...
	/**
	 * Clôture le mois donné : (pas forcément en cours, par exemple en cas de
	 * retard, vacances de l'entreprise) Met à jour les jours travaillés et
	 * congés payés restants de l'année N, décompte ceux de l'année N-1 on
	 * déduit un jour de congé entier pour chaque absence. Par exemple lors des
	 * vacances, pour savoir combien de jours de congés payés sont consommés,
	 * même si ladite absence dure seulement une demi-journée. Si dernier mois de
	 * l'année, clôture aussi l'année
	 * 
	...
	public void clotureMois(SalarieAideADomicile salarieAideADomicile, double joursTravailles) throws SalarieException {
		...
		salarieAideADomicile.setCongesPayesAcquisAnneeN(
				salarieAideADomicile.getCongesPayesAcquisAnneeN() + salarieAideADomicile.CONGES_PAYES_ACQUIS_PAR_MOIS);
	...
	/**
	 * Clôture l'année donnée. Il s'agit d'une année DE CONGES donc du 1er juin
	 * au 31 mai. Passe les variables N à N-1
	 * 
	 * @param salarieAideADomicile
	 */
	void clotureAnnee(SalarieAideADomicile salarieAideADomicile) {
		...
		// on ne garde que les jours de congés pris sur la nouvelle année (voir
		// ajouteCongés()) :
		salarieAideADomicile.setCongesPayesPris(new LinkedHashSet<>(salarieAideADomicile.getCongesPayesPris().stream()
				.filter(d -> d.isAfter(LocalDate.of(
						Entreprise.getPremierJourAnneeDeConges(salarieAideADomicile.getMoisEnCours()).getYear(), 5,
						31)))
				.collect(Collectors.toList())));
